...
def calc_mpm_group_xmm(base_dir, roi, subject_list, max_cl_num, method, mpm_thres, njobs):
    ...
    # 打印 REF_IMG 统计
    labels, counts = np.unique(ref_data, return_counts=True)
    print("REF_IMG label counts:")
    ...
    for cl_num in range(2, max_cl_num + 1):
        ...
        nib.save(
            ...
            os.path.join(
                prob_dir, f"{roi}_{cl_num}_MPM_thr{int(mpm_thres*100)}.nii.gz"
            )
        )
...
if __name__ == "__main__":
    ...
    calc_mpm_group_xmm(
        ...
        roi        = args.roi_name,
        subject_list = args.subject_data,
