...
for dataset in "${datasets[@]}"; do
    ...
    for sub_id in sub-*; do
        ...
        for i in {1..50}; do
            fslmaths data/jhu48_native.nii.gz -thr $i -uthr $i -bin data/seed_region_$i.nii.gz
        done
        ...
        if [ -d "data.bedpostX" ]; then
            ...
            for i in {1..50}; do
                ...
                probtrackx2_gpu \
                    --seed=../data/seed_region_$i.nii.gz \  # Use individual seed files
                    --mask=../data/nodif_brain_mask.nii.gz \
                    --samples=merged \
                    --dir=../data/probtrack_output/seed_$i \  # Output directory specific to each seed
                    --forcedir \
                    --opd
