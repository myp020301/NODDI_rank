...
for dataset in "${datasets[@]}"; do
    ...
    for sub_id in sub-*; do
        ...
        applywarp -i $JHU48_SEED \
                  ...
                  -o data/jhu48_native_nn.nii.gz \
                  --interp=nn

        # Create individual seed region files (1 to 50)
        for i in {1..50}; do
            fslmaths data/jhu48_native_nn.nii.gz -thr $i -uthr $i -bin data/seed_region_$i.nii.gz
        done

        for i in {1..50}; do
            echo "../data/seed_region_$i.nii.gz" >> data/seed_list.txt
        done

        # 11) Run probtrackx2_gpu for each seed region (1 to 50)
        if [ -d "data.bedpostX" ]; then
            cd data.bedpostX

            echo "[INFO] Starting probtrackx2_gpu for seed region $i..."

            # Run probtrackx2_gpu for each seed region file
            probtrackx2_gpu \
                --seed=../data/seed_list.txt\
                --mask=../data/nodif_brain_mask.nii.gz \
                --samples=merged \
                --targetmasks=../data/seed_list.txt\
                --dir=../data/probtrack_os2t/ \
                --forcedir \
                --os2t\
                --opd
            ...
            for X in {0..49}; do
                INPUT_FILES=""
                for Y in {1..50}; do
                    INPUT_FILES="${INPUT_FILES} ../data/probtrack_os2t/seeds_${X}_to_seed_region_${Y}.nii.gz"
                done
                fslmerge -t ../data/probtrack_os2t/seeds_$((X+1))_to_seed_all.nii.gz $INPUT_FILES
                echo "[INFO] Merged seed_region_$((X+1)) into a 4D file."
            done

            echo "[INFO] Finished probtrackx2_gpu for seed region $i."
            cd ..
        else
             echo "[ERROR] bedpostx output folder 'data.bedpostX' not found."
        fi
