...
def train(train_loader, model, optimizer, scheduler, edge_index, device, is_voxel, model_name,criterion_rank_moco,criterion_dif_moco):
    ...
    for i, data in tqdm(enumerate(train_loader, 0), total=len(train_loader), desc='train'):
        ...
        if is_voxel:
            ...
            with torch.profiler.record_function("prepare_sub_batches"):
            ...
            with torch.profiler.record_function("compute_ranking_and_diff_loss"):
                for sub_batch_idx in range(num_sub_batches):
                    ...
                    # 计算 rankingloss
                    for idx in range(3):
                        out_idx = out_sub[:, idx].unsqueeze(1)
                        label_idx = label_sub[:, idx].unsqueeze(1)
                        rank_loss = criterion_rank_moco(out_idx, label_idx)
                        total_ranking_loss += rank_loss
        ...
        # 打印或记录损失值
        print(f"MSE Loss: {mse_loss.item()}, Ranking Loss: {total_ranking_loss.item()}, Diff Loss: {total_diff_loss.item()}")    
        loss.backward()
        optimizer.step()
...
def main(args):
    ...
    with torch.profiler.profile(
        ...
        for epoch in range(0, data_cfg['train']['epoches']):
            total_loss, total_index1_loss, total_index2_loss, total_index3_loss, epoch_time_cost = train(
                train_loader,
                model,
                optimizer,
                scheduler,
