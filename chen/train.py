...
def parse_args():
    ...
    parser.add_argument("--train_subject_id", default=[    "100206", "100307", "100408", "100610", "101006",
    "101107", "101309", "101410", "101915", "102008",
    "102109", "102311", "102513", "102614", "102715",
    ],
                        type=list, help="Training file name")
    parser.add_argument("--valid_subject_id", default=["102816", "103010", "103111", "103212", "103414"],
...
def train(train_loader, model, optimizer, scheduler, edge_index, device, is_voxel, model_name,criterion_rank_moco,criterion_dif_moco):
    ...
    for i, data in tqdm(enumerate(train_loader, 0), total=len(train_loader), desc='train'):
        ...
        if is_voxel:
            ...
            # 计算 rankingloss
            for idx in range(3):
                out_idx = out_sub_batches[:, :,idx]
                label_idx = label_sub_batches[:, :, idx]
