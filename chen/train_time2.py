...
def train(train_loader, model, optimizer, scheduler, edge_index, device, is_voxel, model_name, criterion_rank_moco, criterion_dif_moco):
    ...
    # 初始化 Profiler，仅针对 train 函数内部
    with profile(
        activities=[ProfilerActivity.CPU, ProfilerActivity.CUDA],
        record_shapes=True,
        profile_memory=True,
        with_stack=True  # 如果不需要堆栈信息，可以设为 False 减少开销
    ) as prof:
        for i, data in tqdm(enumerate(train_loader, 0), total=len(train_loader), desc='train'):
            ...
            with record_function("model_forward"):
                out = model(input, edge_index) if edge_index is not None else model(input)

            # 计算 MSE 损失
            with record_function("mse_loss_computation"):
            ...
            ranking_loss_weight = 1  # 根据需要调整
            diff_loss_weight = 1     # 根据需要调整

            if is_voxel:
                with record_function("voxel_loss_computation"):
                    index1_loss = F.mse_loss(out[:, 0], label[:, 0])
                    index2_loss = F.mse_loss(out[:, 1], label[:, 1])
                    index3_loss = F.mse_loss(out[:, 2], label[:, 2])
            else:
                with record_function("ranking_diff_loss_computation"):
                    index1_loss = F.mse_loss(out[:, :, :, 0], label[:, :, :, 0])
                    index2_loss = F.mse_loss(out[:, :, :, 1], label[:, :, :, 1])
                    index3_loss = F.mse_loss(out[:, :, :, 2], label[:, :, :, 2])
                    ...
                    # 定义子批次大小和采样数量
                    sub_batch_size = 64
                    num_sub_batches = 10  # 减少子批次数量以优化性能
                    ...
                    for sub_batch_idx in range(num_sub_batches):
                        ...
                        # 计算 rankingloss
                        with record_function(f"ranking_loss_sub_batch_{sub_batch_idx+1}"):
                        ...
                        # 计算 diffloss
                        with record_function(f"diff_loss_sub_batch_{sub_batch_idx+1}"):
                            diff_loss = criterion_dif_moco(out_sub.unsqueeze(2), label_sub.unsqueeze(2))
                            total_diff_loss += diff_loss

            # 加权损失
            loss += ranking_loss_weight * total_ranking_loss
            loss += diff_loss_weight * total_diff_loss
            ...
            # 打印损失值
            print(f"MSE Loss: {mse_loss.item():.6f}, Ranking Loss: {total_ranking_loss.item():.6f}, "
                  f"Diff Loss: {total_diff_loss.item():.6f}")
        prof.step()
    # 训练完成后，打印 Profiler 的结果
    print(prof.key_averages().table(sort_by="cuda_time_total", row_limit=10))
    print(prof.key_averages().table(sort_by="self_cpu_memory_usage", row_limit=10))
    # 获取按 CUDA 总时间排序的前10个操作及其执行次数
    top_operations = prof.key_averages().sort_by("cuda_time_total")[:10]

# 打印每个操作的名称、cuda_time_total 和执行次数
    for entry in top_operations:
       print(f"Operation: {entry.key}, CUDA Time Total: {entry.cuda_time_total:.2f} ms, Count: {entry.count}")
