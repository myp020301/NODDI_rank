...
def restore_img(subject_id, gt, prediction, is_voxel, args):
    ...
    y_size = mask.shape[1]
    z_size = mask.shape[2]
    gt_like = np.zeros([x_size, y_size, z_size, 3], dtype=np.float32)
    prediction_like = np.zeros([x_size, y_size, z_size, 3], dtype=np.float32)
    psnr_gt = []
    psnr_prediction = []
    ...
    if is_voxel:
        ...
        for i in range(0, x_size, 1):
            prediction_like[i,edge_distance_x_start:edge_distance_x_ed, edge_distance_y_start:edge_distance_y_ed,  :] = prediction[i, x_left_add_length:x_left_add_length + x_length, y_left_add_length:y_left_add_length + y_length, :]
            gt_like[i,edge_distance_x_start:edge_distance_x_ed, edge_distance_y_start:edge_distance_y_ed,  :] = gt[i, x_left_add_length:x_left_add_length + x_length, y_left_add_length:y_left_add_length + y_length, :]

        for xx in range(0, x_size, 1):
          for yy in range(edge_distance_x_start, edge_distance_x_ed, 1):
              for zz in range(edge_distance_y_start, edge_distance_y_ed, 1):
                    if mask[xx, yy, zz] > 0:
    ...
    psnr_gt = np.array(psnr_gt, dtype=np.float32)
    psnr_prediction = np.array(psnr_prediction, dtype=np.float32)
    
    if args.is_generate_image:
        save_dir = args.generate_image_save_path
        os.makedirs(save_dir, exist_ok=True)
        
        # 确保预测数据类型与GT一致
        prediction_like = prediction_like.astype(np.float32)
        
        # 如果需要保存为多通道NIfTI，可以直接保存四维图像
        # 但通常建议分开保存每个通道
        for c in range(prediction_like.shape[3]):
            pred_channel = prediction_like[:, :, :, c]
            # 使用掩膜的affine和header
            img_prediction = nib.Nifti1Image(pred_channel, affine, header)
            pred_save_path = os.path.join(save_dir, f"{subject_id}_pred_c{c}.nii.gz")
            nib.save(img_prediction, pred_save_path)
            print(f'Saved prediction channel {c} to {pred_save_path}')
        
        # 如果坚持保存为四维图像，可以这样做，但要确保下游处理能够正确处理
        # img_prediction = nib.Nifti1Image(prediction_like, affine, header)
        # nib.save(img_prediction, save_path)
        # print('Saved 4D prediction image successfully')
