...
def load_training_matrix(allDwiNames, allMaskNames, allFeatureNames, featurenumbers,
    ...
    for iMask in range(len(allMaskNames)):
        ...
        mask = nib.load(allMaskNames[iMask]).get_fdata()
        ###
        ...
        for i in range(mask.shape[0]):
            for j in range(mask.shape[1]):
                for k in range(mask.shape[2]):
                    if mask[i, j, k] > 0:
                        nVox = nVox + 1
    dwi = nib.load(allDwiNames[0]).get_fdata()
    print
    dwiTraining = np.zeros(
        [nPatch, 60, patch_size_low*patch_size_low*patch_size_low])  # [nPatch, dwi.shape[3], patch_size_low*patch_size_low*patch_size_low])
    featurePatchTraining = np.zeros(
        [nPatch, featurenumbers, patch_size_high*patch_size_high*patch_size_high])
    template_value = np.zeros(
        [nPatch,  3, patch_size_high*patch_size_high*patch_size_high])
    ...
    for iMask in range(len(allMaskNames)):
        ...
        for feature_index in range(featurenumbers):
            tempFeature = nib.load(
                allFeatureNames[feature_index][iMask]).get_fdata()
        ...
        for i in range(mask.shape[0]):
            for j in range(mask.shape[1]):
                for k in range(mask.shape[2]):
                    if mask[i, j, k] > 0:
                        for feature_index in range(featurenumbers):
                            featureTraining[nVox,
                                            feature_index] = feature[feature_index][i, j, k]
                        nVox = nVox + 1
    ...
    icvf_data = nib.load(r"/data2/mayupeng/HCP_25_dataset_ori/average_templates/average_icvf.nii.gz").get_fdata()
    isovf_data = nib.load(r"/data2/mayupeng/HCP_25_dataset_ori/average_templates/average_isovf.nii.gz").get_fdata()
    od_data = nib.load(r"/data2/mayupeng/HCP_25_dataset_ori/average_templates/average_od.nii.gz").get_fdata()
    # Setting data
    nPatch = 0
                                            ...
                                            for jj in range(patch_size_high):
                                                for kk in range(patch_size_high):
                                                    if i + i1 + ii >= 0 and i + i1 + ii < mask.shape[0] \
                                                        ...
                                                        featurePatchTraining[nPatch, feature_index, ii *
                                                                             patch_size_high*patch_size_high + jj*patch_size_high + kk] = 0
                                                    template_value[nPatch, 0,ii*patch_size_high*patch_size_high + jj*patch_size_high + kk] = icvf_data[i+i1+ii,j + j1 + jj, k + k1 + kk ] 
                                                    template_value[nPatch, 1,ii*patch_size_high*patch_size_high + jj*patch_size_high + kk] = isovf_data[i+i1+ii,j + j1 + jj, k + k1 + kk]
                                                    template_value[nPatch, 2,ii*patch_size_high*patch_size_high + jj*patch_size_high + kk] = od_data[i+i1+ii,j + j1 + jj, k + k1 + kk]
                                                    
                                                    

                                    i_lr_center = int((i+i1)/upsample +
                                                      (patch_size_high/upsample-1)/2)
                                    j_lr_center = int((j+j1)/upsample +
                                                      (patch_size_high/upsample-1)/2)
                                    k_lr_center = int((k+k1)/upsample +
                                                      (patch_size_high/upsample-1)/2)
                                    ind_patch = 0
                                                    ...
                                                    for indx in range(upsample):
                                                        for indy in range(upsample):
                                                            for indz in range(upsample):
                                                                if mask[int((i_lr_center + ii)*upsample + indx), int((j_lr_center + jj)*upsample + indy), int((k_lr_center + kk)*upsample + indz)] > 0:
                                                                    in_mask = True
                                                                    break
                                                    if in_mask:
                                                        dwiTraining[nPatch, :, ind_patch] \
                                                            = dwi[i_lr_center+ii, j_lr_center+jj, k_lr_center+kk, :60]  # dwi[i_lr_center+ii, j_lr_center+jj, k_lr_center+kk, :]
                                                ind_patch = ind_patch + 1
                                    nPatch = nPatch + 1

    return dwiTraining, featurePatchTraining, scales,template_value
# %%
                                            ...
                                            for kk in range(-patch_size_low//2 + 1, patch_size_low//2 + 1):
                                                ...
                                                if i_lr_center + ii >= 0 and i_lr_center + ii < dwi.shape[0] and j_lr_center + jj >= 0 and j_lr_center + jj < dwi.shape[1] \
                                                    ...
                                                    for indx in range(upsample):
                                                        for indy in range(upsample):
                                                            for indz in range(upsample):
                                                                if mask[(i_lr_center + ii)*upsample + indx, (j_lr_center + jj)*upsample + indy, (k_lr_center + kk)*upsample + indz] > 0:
                                                                    in_mask = True
                                                                    break
                                                    if in_mask:
                                                        dwiTraining[nPatch, :, ind_patch] \
...
def load_test_matrix(dwi, mask, patch_size_high, patch_size_low, upsample):
        ...
        for j in range(0, dwi.shape[1]*upsample, patch_size_high):
            for k in range(0, dwi.shape[2]*upsample, patch_size_high):
                if np.sum(mask[i//upsample:min(dwi.shape[0], i//upsample+patch_size_high//upsample),
                    ...
                    template_value[nPatch,0,0] = icvf_data[i,j,k]
                    template_value[nPatch,1,0] = isovf_data[i,j,k]
                    template_value[nPatch,2,0] = od_data[i,j,k]
                    i_lr_center = int(
                        i/upsample + (patch_size_high/upsample-1)/2)
                    j_lr_center = int(
                        j/upsample + (patch_size_high/upsample-1)/2)
                    k_lr_center = int(
                        k/upsample + (patch_size_high/upsample-1)/2)
                    ind_patch = 0
                    for ii in range(-patch_size_low//2 + 1, patch_size_low//2 + 1):
                        for jj in range(-patch_size_low//2 + 1, patch_size_low//2 + 1):
                            for kk in range(-patch_size_low//2 + 1, patch_size_low//2 + 1):
    ...
    return dwiTest, patchCornerList , template_value
# %%
                ...
                for jj in range(patch_size_high):
                    for kk in range(patch_size_high):
                        if i + ii < rows and j + jj < cols and k + kk < slices:
                            features[i+ii, j+jj, k+kk, feature_index] = featureList[nPatch, feature_index, ii *
                                                                                    patch_size_high*patch_size_high + jj*patch_size_high + kk]*scales[feature_index]
    return features
