...
class Mymodel(nn.Module):
    def __init__(
        ...
        self.Ws_layers = nn.ModuleList([nn.Linear(self.input_shape[1], self.nDictS) for _ in range(self.nonshareLayers)])
        self.Wq_layers = nn.ModuleList([nn.Linear(self.input_shape[0], self.nDictQ) for _ in range(self.nonshareLayers)])
        self.Wfs_layers = nn.ModuleList([nn.Linear(self.input_shape[1], self.nDictS) for _ in range(self.nonshareLayers)])
        self.Wfq_layers = nn.ModuleList([nn.Linear(self.input_shape[0], self.nDictQ) for _ in range(self.nonshareLayers)])
        self.Wis_layers = nn.ModuleList([nn.Linear(self.input_shape[1], self.nDictS) for _ in range(self.nonshareLayers)])
        self.Wiq_layers = nn.ModuleList([nn.Linear(self.input_shape[0], self.nDictQ) for _ in range(self.nonshareLayers)])
        
        self.W_fx_layers_S = nn.ModuleList([nn.Linear(self.nDictS, self.nDictS) for _ in range(self.nonshareLayers)])
        self.W_fx_layers_Q = nn.ModuleList([nn.Linear(self.nDictQ, self.nDictQ) for _ in range(self.nonshareLayers)])
        self.W_ix_layers_S = nn.ModuleList([nn.Linear(self.nDictS, self.nDictS) for _ in range(self.nonshareLayers)])
        self.W_ix_layers_Q = nn.ModuleList([nn.Linear(self.nDictQ, self.nDictQ) for _ in range(self.nonshareLayers)])
        self.S_layers_S = nn.ModuleList([nn.Linear(self.nDictS, self.nDictS) for _ in range(self.nonshareLayers)])
        self.S_layers_Q = nn.ModuleList([nn.Linear(self.nDictQ, self.nDictQ) for _ in range(self.nonshareLayers)])
    ...
    def forward(self, x):
        ...
        for i in range(self.nonshareLayers):
            Ws_output = self.permute(self.Ws_layers[i](x))
            Wq_output = self.permute(self.Wq_layers[i](Ws_output))
            Wfs_output = self.permute(self.Wfs_layers[i](x))
            Wfq_output = self.permute(self.Wfq_layers[i](Wfs_output))
            Wis_output = self.permute(self.Wis_layers[i](x))
            Wiq_output = self.permute(self.Wiq_layers[i](Wis_output))
        ...
        for layer in self.H_layers:
            ...
            H_output = torch.sigmoid(H_output)  # 最后应用激活函数
