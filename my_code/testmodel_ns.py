...
class Mymodel(nn.Module):
    def __init__(
        ...
        self.Ws_layers = nn.Linear(self.input_shape[1], self.nDictS)
        self.Wq_layers = nn.Linear(self.input_shape[0], self.nDictQ)
        self.Wfs_layers = nn.Linear(self.input_shape[1], self.nDictS)
        self.Wfq_layers = nn.Linear(self.input_shape[0], self.nDictQ)
        self.Wis_layers = nn.Linear(self.input_shape[1], self.nDictS)
        self.Wiq_layers = nn.Linear(self.input_shape[0], self.nDictQ)
        
        self.W_fx_layers_S = nn.Linear(self.nDictS, self.nDictS)
        self.W_fx_layers_Q = nn.Linear(self.nDictQ, self.nDictQ)
        self.W_ix_layers_S = nn.Linear(self.nDictS, self.nDictS)
        self.W_ix_layers_Q = nn.Linear(self.nDictQ, self.nDictQ)
        self.S_layers_S = nn.Linear(self.nDictS, self.nDictS)
        self.S_layers_Q = nn.Linear(self.nDictQ, self.nDictQ)
    ...
    def forward(self, x):
        ...
        Ws_output = self.permute(self.Ws_layers(x))
        Wq_output = self.permute(self.Wq_layers(Ws_output))
        Wfs_output = self.permute(self.Wfs_layers(x))
        Wfq_output = self.permute(self.Wfq_layers(Wfs_output))
        Wis_output = self.permute(self.Wis_layers(x))
        Wiq_output = self.permute(self.Wiq_layers(Wis_output))
        
        
        # 在每次迭代中基于更新的 X 计算 W_fx, W_ix, 和 S 的输出
        W_fx_output = self.permute(self.W_fx_layers_Q(self.permute(self.W_fx_layers_S(X))))
        W_ix_output = self.permute(self.W_ix_layers_Q(self.permute(self.W_ix_layers_S(X))))
        S_output = self.permute(self.S_layers_Q(self.permute(self.S_layers_S(X))))
        Diff_X = torch.sub(X, S_output)
        Ctilde = torch.add(Wq_output, Diff_X)
        Wfx_Wfy = torch.add(W_fx_output, Wfq_output)
        F = torch.sigmoid(Wfx_Wfy)
