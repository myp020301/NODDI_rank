...
class Mymodel(nn.Module):
    def __init__(
        ...
        self.W_fx_layers_S = nn.ModuleList([nn.Linear(self.nDictS, self.nDictS) for _ in range(2)])
        self.W_fx_layers_Q = nn.ModuleList([nn.Linear(self.nDictQ, self.nDictQ) for _ in range(2)])
        self.W_ix_layers_S = nn.ModuleList([nn.Linear(self.nDictS, self.nDictS) for _ in range(2)])
        self.W_ix_layers_Q = nn.ModuleList([nn.Linear(self.nDictQ, self.nDictQ) for _ in range(2)])
        self.S_layers_S = nn.ModuleList([nn.Linear(self.nDictS, self.nDictS) for _ in range(2)])
        self.S_layers_Q = nn.ModuleList([nn.Linear(self.nDictQ, self.nDictQ) for _ in range(2)])
