...
from data_loader_tem import load_training_matrix, load_test_matrix, data_combine_matrix
...
# %%
# input ouput
dwiTraining, featurePatchTraining, scales, template_value = load_training_matrix(
    allDwiNames,
    allMaskNames,
    allFeatureNames,
    featurenumbers,
...
if not os.path.exists(model_name):
    ...
    template_tensor = torch.from_numpy(template_value).float()
    train_dataset = TensorDataset(X_train_tensor, y_train_tensor,template_tensor)
    ...
    for epoch in range(epochs):
        ...
        running_loss = 0.0
        for step, (batch_x, batch_y,template) in loop:
            batch_x, batch_y, template = batch_x.to(device), batch_y.to(device),template.to(device)
        ...
        with torch.no_grad():
            for step, (batch_x, batch_y,template) in loop2:
                batch_x, batch_y,template = batch_x.to(device), batch_y.to(device),template.to(device)
...
for iMask in trange(len(allTestDwiNames)):
    ...
    dwiTest, patchCornerList ,template_value = load_test_matrix(
        dwi, mask, patch_size_high, patch_size_low, upsample
    )
    dwiTest_tensor = torch.from_numpy(dwiTest).float()
    template_tensor = torch.from_numpy(template_value).float()
    test_dataset = TensorDataset(dwiTest_tensor,template_tensor)
    ...
    with torch.no_grad():
        for step, (batch_x,template) in loop2:
