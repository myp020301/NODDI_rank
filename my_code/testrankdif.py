...
from different_loss import RelativeDifferenceLoss
...
coe_rank = 1
coe_dif = 1
print("coe_rank",coe_rank)
print("coe_dif",coe_dif)
...
if not os.path.exists(model_name):
    ...
    criterion_rank = rank_difference_loss(
        sorter_checkpoint_path="/data2/mayupeng/Tied_rank_best_lstmla_slen_128.pth.tar"
    ).to(device)
    criterion_dif = RelativeDifferenceLoss().to(device)
    epochs = 10
    ...
    for epoch in range(epochs):
        ...
        for step, (batch_x, batch_y) in loop:
            ...
            loss2 = criterion_rank(output[:, 0, :], batch_y[:, 0, :]) + criterion_rank(output[:, 1, :], batch_y[:, 1, :]) +criterion_rank(output[:, 2, :], batch_y[:, 2, :])
            loss3 = criterion_dif(output, batch_y)
            loss = loss1  + coe_rank * loss2  + coe_dif * loss3
            loss.backward()
