...
if os.path.exists(residual_model_name) == False:
    dwiTraining = load_training_matrix_boot(
        allDwiNames, allMaskNames, patch_size_high, patch_size_low, upsample
    )
    residual_computation = dict_computation(
        regressor, dwiTraining.shape[1:], nDictQ, nDictS
    )
    epoch = 10
    hist = residual_computation.fit(
        dwiTraining,
        dwiTraining,
        batch_size=128,
        epochs=epoch,
...
if test > 0:
    ...
    for iMask in trange(startInd, endInd, desc="subject"):
        ...
        # Save residuals
        residuals_npy_name = os.path.join(
            directory, "residuals" + "_sub_" + "%02d" % iMask
        )
        ...
        for rep in range(1):
            ...
            for sample in range(residual.shape[0]):
                ...
                sampled_residual = np.random.choice(
                    residual_flatten, residual.shape[1] * residual.shape[2]
                )
                YBoot[sample] = YRecList[sample] + np.reshape(
                    sampled_residual, (residual.shape[1], residual.shape[2])
                )
            featureBootList = regressor.predict(YBoot)

        featuresBootAll = np.zeros(
            [
                repetition,
                featureBootList.shape[0],
                featureBootList.shape[1],
                featureBootList.shape[2],
            ]
        )
        ...
        featuresBootMeanImage = data_combine_matrix(
            featureBootMean,
            mask.shape,
            upsample,
            patch_size_high,
        ...
        featuresBootStdImage = data_combine_matrix(
            featureBootStd,
            mask.shape,
            upsample,
            patch_size_high,
        ...
        for feature_index in range(featuresBootMeanImage.shape[-1]):
            feature_boot_mean_nii = nib.Nifti1Image(
                featuresBootMeanImage[:, :, :, feature_index],
                hdr.get_base_affine(),
                hdr,
            )
            feature_boot_mean_name = os.path.join(
                directory,
                "MESC_sep_dict_reg_feature_boot_mean_"
                + "%02d" % feature_index
                + "_sub_"
                + "%02d" % iMask
                + ".nii.gz",
            ...
            feature_boot_std_nii = nib.Nifti1Image(
                featuresBootStdImage[:, :, :, feature_index], hdr.get_base_affine(), hdr
            )
            feature_boot_std_name = os.path.join(
                directory,
                "MESC_sep_dict_reg_feature_boot_std_"
                + "%02d" % feature_index
                + "_sub_"
                + "%02d" % iMask
                + ".nii.gz",
