...
# %%
def mesc_sep_dict(
    input_shape=[60, 3 * 3 * 3], output_shape=[3, 1 * 1 * 1], nDictQ=300, nDictS=300
):
    nLayers1 = 8
    nLayers2 = 3
    ReLUThres = 0.01
    ...
    Wfq_output = Dense(nDictQ, activation="linear", use_bias=True, name="Wfq")(
        Wfs_output
    )
    ...
    # Ws = Dense(filters=nDictS, activation='relu')
    Wiq_output = Dense(nDictQ, activation="linear", use_bias=True, name="Wiq")(
        Wis_output
    )
