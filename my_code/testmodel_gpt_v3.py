...
class Mymodel(nn.Module):
    def __init__(self):
        ...
        # 残差块层
        self.layer1 = self._make_layer(channels[0], channels[0], blocks=1, stride=1, use_se=True)
        self.layer2 = self._make_layer(channels[0], channels[1], blocks=1, stride=1, use_se=True)
        self.layer3 = self._make_layer(channels[1], channels[2], blocks=1, stride=1, use_se=True)
        self.layer4 = self._make_layer(channels[2], channels[3], blocks=1, stride=1, use_se=True)

        # 使用全局平均池化替代全连接层
        self.avgpool = nn.AdaptiveAvgPool3d(1)  # 输出尺寸为 (batch_size, channels[3], 1, 1, 1)
    ...
    def forward(self, x):
        ...
        # 添加 Sigmoid 激活函数并乘以 10，将输出限制在 (0,10)
